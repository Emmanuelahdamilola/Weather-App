...
async function fetchWeather(city) {
  ...
  try {
    const response = await fetch(
      `${apiUrl}${encodeURIComponent(city)}&appid=${key}`,
    );
    ...
    document.querySelector(
      ".city",
    ).innerHTML = `${cityName}, <span class="country">${country}</span>`;
    ...
    document.getElementById("outfitSuggestion").innerText = getOutfitSuggestion(
      temp,
      condition,
      cityName,
    );
    ...
    try {
      const forecastResponse = await fetch(
        `${forecastApiUrl}${encodeURIComponent(
          city,
      ...
      if (forecastResponse.ok) {
        ...
        document.getElementById(
          "weeklySummary",
        ).innerText = generateWeeklySummary(forecastData.list, cityName);
      }
    } catch {
...
async function updateSubWeatherCards() {
  ...
  for (let i = 0; i < cities.length; i++) {
    try {
      const response = await fetch(
        `${apiUrl}${encodeURIComponent(cities[i])}&appid=${key}`,
      );
      ...
      card.querySelector(".sub-temp").textContent = `${Math.round(
        data.main.temp,
      )}°C`;
      ...
      card.querySelector(".sub-wind-speed").textContent = `${Math.round(
        data.wind.speed,
      )} m/s`;
      ...
      card.querySelector(".sub-weather-icon").src = getWeatherIcon(
        data.weather[0].main,
      );
    } catch (err) {
    ...
    // Fetch forecast for weekly summary
    const forecastResponse = await fetch(
      `${forecastApiUrl}${city}&units=metric&appid=${window.API_KEY}`,
    );
    const forecastData = await forecastResponse.json();
    document.getElementById("weeklySummary").innerText = generateWeeklySummary(
      forecastData.list,
      city,
    );
  }
...
async function updateSubWeatherCards() {
  ...
  for (let i = 0; i < cities.length; i++) {
    const response = await fetch(
      `${apiUrl}${cities[i]}&appid=${window.API_KEY}`,
    );
    const data = await response.json();
    const card = cards[i];
    if (!card) continue;
...
function getOutfitSuggestion(temp, condition, city) {
  ...
  if (c.includes("rain")) {
    advice =
      "Rain is likely — carry an umbrella and wear something waterproof. Avoid suede shoes. A native kaftan with a jacket works too.";
  } else if (c.includes("snow")) {
    advice = "It's snowy. Bundle up with a coat, gloves, and boots.";
  } else if (c.includes("clear") && temp >= 30) {
    advice =
      "Very hot and sunny — wear light cotton or short-sleeve Ankara. Use sunglasses and stay hydrated.";
    ...
    advice =
      "Weather seems moderate. Wear comfortable clothes and check for updates.";
  }
...
function generateWeeklySummary(forecastList, city) {
  ...
  const [mostCommon] = Object.entries(conditionFreq).sort(
    (a, b) => b[1] - a[1],
  )[0] || [null];
